...
# mutilado: la unidad medico-quirurgica quedaba cortada a mitad de su sigla, y
# un cruce por nombre contra el perfil de unidades no la encontraba.
...
tab <- do.call(rbind, lapply(resultados, function(r)
  data.frame(unidad = r$unidad, n = r$n,
             ...
             row.names = NULL)))
print(transform(tab, unidad = corto(unidad, 40)), row.names = FALSE)
cat("\nNominal:", 1 - ALFA, "\n")
